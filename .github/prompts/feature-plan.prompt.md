...
description: Create an approved feature plan package with tasks, analysis, and governance review.
---

Create a feature planning package only (no implementation):
...
Return the mandatory outputs from Feature-Planner, including tasks.md-style decomposition, parallel lanes, analysis findings, executor handoff, artifact path, and final `status.json` summary.
...
- Auto-invoke `Feature-Researcher` when score `< 70`, or risk is `medium/high` with incomplete evidence, or architecture seams/external precedent are unclear.
