...
Example:
  ...
  puzzle_manager run --source ogs   Now requires explicit --source
    ...
    # adapter-scaffold command (Theme 12)
    adapter_scaffold_parser = subparsers.add_parser(
        "adapter-scaffold",
        help="Generate a new adapter package + sources.json stub (Theme 12).",
    )
    adapter_scaffold_parser.add_argument(
        "--id", required=True, dest="new_id",
        help="New adapter id (also the source id and Python package name).",
    )
    adapter_scaffold_parser.add_argument(
        "--kind", default="local", choices=["local"],
        help="Template kind. V1 supports 'local'.",
    )
    adapter_scaffold_parser.add_argument(
        "--name", default=None, dest="new_name",
        help="Display name for the source. Defaults to the id.",
    )
    adapter_scaffold_parser.add_argument(
        "--path", default=None, dest="new_path",
        help="Optional path to seed the local config block with.",
    )
    adapter_scaffold_parser.add_argument(
    ...
    adapter_scaffold_parser.add_argument(
        "--adapters-dir", default=None, dest="adapters_dir",
...
# ---------------------------------------------------------------------------
# Theme 12: adapter-scaffold — generate a new adapter package + sources.json
# stub. V1 supports `--kind local`: writes a thin LocalAdapter wrapper that
# registers under the new id. The apply path goes through PipelineLock + the
# same sources.json schema validation as `adapter-config add`.
# ---------------------------------------------------------------------------

_ADAPTER_ID_RE = re.compile(r"^[a-z][a-z0-9_-]{0,63}$")


def _adapter_scaffold_template_local(new_id: str, class_name: str) -> tuple[str, str]:
    """Return (init_py, adapter_py) for a `local`-kind scaffold."""
    init_py = (
        f'"""Scaffolded adapter package for source {new_id!r}."""\n\n'
        f"from .adapter import {class_name}\n\n"
        f'__all__ = ["{class_name}"]\n'
    )
    adapter_py = (
        f'"""Scaffolded local-kind adapter for source {new_id!r}.\n\n'
        f"Generated by `puzzle_manager adapter-scaffold --kind local --id {new_id}`.\n"
        f"This is a thin subclass of ``LocalAdapter`` registered under the new id\n"
        f"so checkpoints, ingest state, and publish-log entries are scoped to\n"
        f"this source. Edit freely once you start customising fetch/parse logic.\n"
        f'"""\n\n'
        f"from backend.puzzle_manager.adapters._registry import register_adapter\n"
        f"from backend.puzzle_manager.adapters.local.adapter import LocalAdapter\n\n\n"
        f'@register_adapter("{new_id}")\n'
        f"class {class_name}(LocalAdapter):\n"
        f'    """Auto-generated local adapter for {new_id!r}."""\n\n'
        f"    @property\n"
        f"    def name(self) -> str:\n"
        f'        return "{new_id}"\n\n'
        f"    @property\n"
        f"    def source_id(self) -> str:\n"
        f'        return "{new_id}"\n'
    )
    return init_py, adapter_py
...
def cmd_adapter_scaffold(args: argparse.Namespace) -> int:
    ...
    from backend.puzzle_manager.adapters._registry import (
        discover_adapters, list_adapters,
    )
    from backend.puzzle_manager.core.atomic_write import atomic_write_json
    from backend.puzzle_manager.pipeline.lock import PipelineLock, PipelineLockError
    ...
    def _emit(payload: dict, rc: int) -> int:
        if args.json:
            ...
            print(f"{'OK' if payload.get('ok') else 'FAIL'}: {payload.get('message', '')}")
        return rc

    new_id: str = args.new_id
    kind: str = args.kind
    new_name: str = args.new_name or new_id
    new_path: str | None = args.new_path
    ...
    if not _ADAPTER_ID_RE.match(new_id):
        base_payload["errors"].append({
            "code": "invalid-id",
            "message": "id must match [a-z][a-z0-9_-]{0,63}",
        })
        base_payload["message"] = "invalid adapter id"
        return _emit(base_payload, 2)

    discover_adapters()
    if new_id in list_adapters():
        base_payload["errors"].append({
            "code": "id-collision",
            "message": f"adapter id '{new_id}' already registered",
        })
        base_payload["message"] = "id already registered"
        return _emit(base_payload, 2)

    adapters_root = (
        Path(args.adapters_dir) if getattr(args, "adapters_dir", None)
        else Path(__file__).parent / "adapters"
    )
    pkg_dir = adapters_root / new_id
    if pkg_dir.exists():
        base_payload["errors"].append({
            "code": "pkg-exists",
            "message": f"adapter package directory already exists: {pkg_dir}",
        })
        base_payload["message"] = "package directory exists"
        return _emit(base_payload, 2)

    class_name = "".join(part.capitalize() for part in re.split(r"[_-]", new_id)) + "Adapter"
    init_py, adapter_py = _adapter_scaffold_template_local(new_id, class_name)

    sources_entry: dict = {
        "id": new_id,
        "name": new_name,
        "adapter": new_id,
        "config": {"path": new_path} if new_path else {},
    }
    base_payload["sources_entry"] = sources_entry
    base_payload["files_created"] = [
        (pkg_dir / "__init__.py").as_posix(),
        (pkg_dir / "adapter.py").as_posix(),
    ]
    ...
    new_sources_doc = {
        "active_adapter": raw_sources.get("active_adapter", ""),
        "sources": list(raw_sources.get("sources", [])) + [sources_entry],
    }
    issues = _validate_sources_doc(new_sources_doc)
    if issues:
        base_payload["errors"].extend(issues)
        base_payload["message"] = "schema validation failed"
        return _emit(base_payload, 2)

    if dry_run:
        base_payload["ok"] = True
        base_payload["message"] = f"would scaffold adapter '{new_id}' (dry-run)"
        return _emit(base_payload, 0)

    lock = PipelineLock(run_id="adapter-scaffold")
    try:
        if not force:
            lock.acquire()
    except PipelineLockError as exc:
        base_payload["errors"].append({
            "code": "pipeline-locked",
            "message": str(exc),
        })
    ...
    try:
        ...
        (pkg_dir / "adapter.py").write_text(adapter_py, encoding="utf-8")
        atomic_write_json(sources_path, new_sources_doc)
    finally:
        if not force:
            lock.release()

    base_payload["ok"] = True
    base_payload["message"] = f"scaffolded adapter '{new_id}' ({kind})"
    return _emit(base_payload, 0)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    elif args.command == "adapter-scaffold":
