...
def publish_log_dir(tmp_path) -> Path:
    ...
    # Create a day's worth of publish log entries
    entries = [
        {"run_id": "20260130-abc12345", "puzzle_id": "puzzle-001", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/puzzle-001.sgf", "tags": ["life-and-death", "tesuji"], "quality": 1, "trace_id": "t001", "level": "beginner", "collections": []},
        {"run_id": "20260130-abc12345", "puzzle_id": "puzzle-002", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/puzzle-002.sgf", "tags": ["life-and-death"], "quality": 1, "trace_id": "t002", "level": "beginner", "collections": []},
        {"run_id": "20260130-abc12345", "puzzle_id": "puzzle-003", "source_id": "ogs", "path": "sgf/intermediate/2026/01/batch-001/puzzle-003.sgf", "tags": ["ko", "ladder"], "quality": 1, "trace_id": "t003", "level": "intermediate", "collections": []},
    ]
...
def output_dir_with_sgf(tmp_path, publish_log_dir) -> Path:
    ...
    for puzzle_id, level, tags in [
        ...
        sgf_dir = output_dir / "sgf" / level / "2026" / "01" / "batch-001"
...
class TestExtractLevelFromPath:
    ...
    def test_extracts_beginner_level(self):
        ...
        path = "sgf/beginner/2026/01/batch-001/puzzle-001.sgf"
    ...
    def test_extracts_intermediate_level(self):
        ...
        path = "sgf/intermediate/2026/01/batch-001/puzzle-002.sgf"
...
class TestRebuildProducesAccurateCounts:
    ...
    def test_rebuild_counts_by_tag(self, output_dir_with_sgf):
        """Rebuild correctly counts puzzles per tag."""
        inventory = rebuild_inventory(output_dir=output_dir_with_sgf)
    ...
    def test_rebuild_handles_missing_sgf_files(self, publish_log_dir):
        ...
        Spec 107, T021/FR-012: Entries without corresponding SGF files
        are ghost entries from rollback and MUST be skipped during rebuild.
        """
        # SGF files don't exist, but publish log does
        output_dir = publish_log_dir.parent

        inventory = rebuild_inventory(output_dir=output_dir)

        # Spec 107: Ghost entries (missing files) are skipped
        # Since all 3 entries have missing files, count should be 0
        assert inventory.collection.total_puzzles == 0
        assert inventory.collection.by_puzzle_level == {}

        # Tags should also be empty (no files to parse)
        assert inventory.collection.by_tag == {}
...
class TestRebuildQualityBreakdown:
    ...
    def publish_log_with_quality(self, tmp_path) -> Path:
        ...
        entries = [
            {"run_id": "test", "puzzle_id": "p1", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p1.sgf", "quality": 3, "tags": ["life-and-death"], "trace_id": "t1", "level": "beginner", "collections": []},
            {"run_id": "test", "puzzle_id": "p2", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p2.sgf", "quality": 4, "tags": ["life-and-death"], "trace_id": "t2", "level": "beginner", "collections": []},
            {"run_id": "test", "puzzle_id": "p3", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p3.sgf", "quality": 3, "tags": ["life-and-death"], "trace_id": "t3", "level": "beginner", "collections": []},
            {"run_id": "test", "puzzle_id": "p4", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p4.sgf", "quality": 5, "tags": ["life-and-death"], "trace_id": "t4", "level": "beginner", "collections": []},
            {"run_id": "test", "puzzle_id": "p5", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p5.sgf", "quality": 2, "tags": ["life-and-death"], "trace_id": "t5", "level": "beginner", "collections": []},
        ]
        ...
        sgf_dir = tmp_path / "sgf/beginner/2026/01/batch-001"
    ...
    def test_rebuild_defaults_missing_quality_to_1(self, tmp_path):
        """Rebuild defaults missing quality to 1 (Unverified)."""
        ops_dir = tmp_path / ".puzzle-inventory-state"
        ops_dir.mkdir()
        log_dir = ops_dir / "publish-log"
        log_dir.mkdir()

        # Entries with explicit quality values (clean-slate: all fields mandatory)
        entries = [
            {"run_id": "test", "puzzle_id": "p1", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p1.sgf", "quality": 1, "tags": ["life-and-death"], "trace_id": "t1", "level": "beginner", "collections": []},
            {"run_id": "test", "puzzle_id": "p2", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p2.sgf", "quality": 1, "tags": ["life-and-death"], "trace_id": "t2", "level": "beginner", "collections": []},
            {"run_id": "test", "puzzle_id": "p3", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p3.sgf", "quality": 3, "tags": ["life-and-death"], "trace_id": "t3", "level": "beginner", "collections": []},
        ]
        ...
        # Spec 107: Create corresponding SGF files
        sgf_dir = tmp_path / "sgf/beginner/2026/01/batch-001"
        sgf_dir.mkdir(parents=True)
        sgf_content = "(;FF[4]GM[1]SZ[19]YG[beginner]YT[life-and-death])"
        for entry in entries:
        ...
        # 2 entries with quality 1, 1 with quality 3
    ...
    def test_rebuild_initializes_all_quality_keys(self, tmp_path):
        ...
        entries = [
            {"run_id": "test", "puzzle_id": "p1", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p1.sgf", "quality": 3, "tags": [], "trace_id": "t1", "level": "beginner", "collections": []},
        ]
...
class TestRebuildSkipsGhostEntries:
    ...
    def test_rebuild_skips_missing_sgf_files(self, tmp_path):
        ...
        entries = [
            ...
            {"run_id": "test", "puzzle_id": "p2", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/p2.sgf", "tags": ["life-and-death"], "quality": 1, "trace_id": "t2", "level": "beginner", "collections": []},  # Ghost
        ...
        p1_path = tmp_path / "sgf/beginner/2026/01/batch-001"
        p1_path.mkdir(parents=True)
        (p1_path / "p1.sgf").write_text(sgf_content, encoding="utf-8")

        p3_path = tmp_path / "sgf/intermediate/2026/01/batch-001"
    ...
    def test_rebuild_with_all_ghost_entries(self, tmp_path):
        ...
        entries = [
            {"run_id": "test", "puzzle_id": "ghost1", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/ghost1.sgf", "tags": [], "quality": 1, "trace_id": "t1", "level": "beginner", "collections": []},
            {"run_id": "test", "puzzle_id": "ghost2", "source_id": "ogs", "path": "sgf/beginner/2026/01/batch-001/ghost2.sgf", "tags": [], "quality": 1, "trace_id": "t2", "level": "beginner", "collections": []},
        ]
