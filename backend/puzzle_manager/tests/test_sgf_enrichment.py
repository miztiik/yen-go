...
- GN standardization
- Root comment removal (preserving move comments)
- Empty line removal
...
class TestRootCommentHandling:
    """Verify root comment handling.

    Since spec-103 Phase 12 + preserve_root_comment=True default,
    root C[] is PRESERVED in enriched output. Move comments in
    variations are also preserved by the solution tree copying.
    """

    def test_root_comment_removed(self, analyze_stage):
        """Root-level C[] should be preserved (preserve_root_comment=True default)."""
        ...
        assert "C[This is a puzzle" in result
...
class TestYenGoPropertyInjection:
    ...
    def test_existing_yengo_properties_overwritten(self, analyze_stage):
        """Existing YG, YT properties are overwritten with new values."""
        ...
        # Old values should not be present (overwritten)
        # Note: SGFBuilder.from_game() preserves existing yengo_props,
        # but we explicitly set new values which override
...
AW[ff])"""
        ...
        # Root comment preserved (preserve_root_comment=True default)
        assert "C[Root comment" in result
