...
| `cli.py` | 14-command argparse CLI: `run`, `status` (Theme 2a: `--failures-summary`), `sources`, `daily`, `clean`, `validate`, `publish-log`, `logs` (Theme 4: `grep`), `rollback`, `vacuum-db`, `inventory` (Theme 14a: `--check --json` emits `IntegrityReport`; Theme 14c1: `--{rebuild,reconcile,fix} --dry-run --json` emits `InventoryMutationPreview`), `runtime-info` (Theme 3a), `activity` (Theme 13a) |
...
| `models/inventory_preview.py` | `InventoryMutationPreview` (Pydantic, `op = rebuild|reconcile|fix`) — Theme 14c1 wire contract for `inventory --{rebuild,reconcile,fix} --dry-run --json`. Reports `disk_total`, `snapshot_total_before`, `delta`, `would_rewrite_snapshot`, `would_rebuild_search_db`, `fix_skip_reason` without touching disk. |
