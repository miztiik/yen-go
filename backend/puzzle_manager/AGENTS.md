...
| `models/ops_catalog.py` | `OpsCatalogEntry` (Pydantic) + `OPS_CATALOG` registry + `get_ops_catalog()` — Theme 16a wire contract for `ops catalog --json`. Single source of truth for blast-radius classification (scope, reversible, preview_supported, section) of every mutating CLI subcommand. Theme 11 sub-slice 4b adds `tags rename --apply` / `tags merge --apply` / `levels rename --apply` rows (`destructive`, `reversible=False`, `scope=[published_corpus, files]`). The drift-fence test in `tests/unit/test_ops_catalog.py` enforces that registering a new mutating subcommand without a catalog row fails CI. |
...
| `inventory/taxonomy_mutations.py` | Theme 11 apply-path writers: `apply_tags_rename(old, new, *, sgf_root=None, config_dir=None)`, `apply_tags_merge(sources, target, ...)`, `apply_levels_rename(old, new, ...)` — regex-rewrite root `YT[]` / `YG[]` across every published SGF, then `atomic_write_json` the affected config (`config/tags.json` or `config/puzzle-levels.json`). Returns `TaxonomyApplyResult{op, sources, target, files_scanned, files_rewritten, config_updated}`. Caller (`cli.py`) wraps with `PipelineLock` + audit row. Test seams: `sgf_root` / `config_dir` kwargs override `get_sgf_output_dir()` / `get_project_root()/config`. |
