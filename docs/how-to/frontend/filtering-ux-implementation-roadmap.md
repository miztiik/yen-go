...
1. Map puzzle IDs to indexes via the loader's entry list

1. Pass `initialCompletedIndexes: Set<number>` prop to `PuzzleSetPlayer`

1. `PuzzleSetPlayer` uses this as initial state for `completedIndexes`
...
1. Add `FilterBar` with: `All` | `In Progress` | `Completed` | `Not Started`

1. Add sort `FilterBar` with: `Featured` (default) | `Name` | `Recently Played`

1. "In Progress" = has `completed.length > 0 && completed.length < totalPuzzles`

1. "Recently Played" = sort by `lastActivity` descending
...
1. **Status filter on browse pages only** — Collections Browse gets All/In Progress/Complete/Not Started using existing localStorage data

1. **Training Browse already has technique-based slicing** via the Tag filter dropdown + cross-dimensional counts from `useMasterIndexes()`

1. **Technique Browse already has level-based slicing** via the Level filter pills + cross-dimensional counts

1. **"Failed" tracking** requires a small storage addition but is the only new data model needed

1. **No new database tables** — all status filtering is pure client-side against localStorage

1. **localStorage budget is fine** — even at 10K puzzles with per-puzzle tracking, we're at ~2 MB of 5 MB quota

1. **Export/import for backup** — APIs exist but need UI wiring (Settings page button)
