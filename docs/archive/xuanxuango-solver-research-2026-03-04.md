...
- Target page: [http://www.yengo-source.com/solver.htm](http://www.yengo-source.com/solver.htm)
...
1. **Constrained game-tree search**: explore tactical lines primarily inside the region; treat off-region moves as pass/irrelevant in many contexts.

1. **Outcome lattice evaluation**: rank outcomes beyond binary win/loss (unconditional life/death, ko variants, seki, etc.).

1. **Cycle-sensitive handling**: special recognition for double-ko/cyclic states and ko-context outcomes.

1. **Interactive continuation**: after initial solve, respond quickly to user variations using cached/partial analysis state.
