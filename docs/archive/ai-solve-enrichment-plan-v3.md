...
**Historical Status:** SCAFFOLDING COMPLETE, GAPS IDENTIFIED — 220 tests passed but the original review panel audit found 20 implementation gaps.
...
**Remediation Outcome:** The follow-up remediation effort closed 19 of the 20 audit gaps. The remaining live calibration sweep was later retired under current product scope.

**Archive Note (2026-05-08):** Live KataGo calibration is no longer planned for this track. Threshold defaults therefore remain unvalidated by a live sweep and should be treated as implementation defaults, not calibrated guarantees.
...
1. **Tests pass** — Phase-specific tests written and passing. No regressions in existing test suite.

1. **Documentation updated** — All relevant docs reflect the phase's changes.

1. **Review Panel sign-off** — Panel confirms design decisions are correctly implemented, no shortcuts taken, full refactor (no bandaids/stopgaps).
...
1. Build candidate pools from `analyze_position_candidates()`: `correct_pool` contains TE candidates sorted by `(winrate desc, policy desc)`, and `wrong_pool` contains BM/BM_HO candidates sorted by `(policy desc, delta desc)`.

1. Always allocate budget to the primary correct root first.

1. Then allocate to additional roots using configured caps and remaining budget.
...
1. **IF has solution:** validate + discover alternatives (additive only, never delete)

1. **IF position-only:** build full solution tree + refutation branches

1. Difficulty + teaching enrichment
