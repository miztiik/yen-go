...
1. **Private Property Namespace**: `Y*` prefix prevents collision with standard properties

1. **Single Source of Truth**: Config-driven values, never hardcoded

1. **Offline-First**: All data serializable to static files, no runtime computation

1. **Pedagogical Focus**: Properties designed for teaching, not just storage
...
1. **Generated types** - TypeScript types derived from config

1. **Validation consistency** - Same validation in pipeline and frontend
