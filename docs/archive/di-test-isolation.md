...
1. **`.pm-runtime/` directory convention** — All runtime artifacts (staging, state, logs) live under a single root that can be pointed anywhere via the env var

1. **`paths.py` module** — Centralized path resolution that checks `YENGO_RUNTIME_DIR` first, falls back to `.pm-runtime/` at the project root
