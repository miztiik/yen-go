...
1. [Expert Consultation: Go Professionals](#2-expert-consultation-go-professionals)

1. [Expert Consultation: UX Specialists](#3-expert-consultation-ux-specialists)

1. [Gap Analysis](#4-gap-analysis)

1. Recommendations & Action Items

1. Code Bugs & Technical Debt
...
#### Perspective: Educational platform patterns (Khan Academy, Duolingo, Coursera)
...
#### Perspective: Mobile puzzle app patterns (chess.com, lichess, puzzle apps)
...
1. **`PuzzleSetPlayer`** tracks solved/failed in-memory (`completedIndexes`, `failedIndexes`) — but these reset on page reload, not hydrated from localStorage

1. **Collection solve writes to collection store only** — `recordCollectionPuzzleCompletion()` does NOT also write to the global `yen-go-progress`. Two separate stores, not synced.

1. **Training/technique have only aggregate counts** — no per-puzzle tracking, so "unsolved" filtering is impossible without adding per-puzzle tracking for those modes

1. **`ProblemNav` dots** show solved/failed status from in-memory session — lost on reload even though localStorage has the data

1. **No "failed" tracking exists anywhere** — only "completed" (success). A puzzle attempted but never solved leaves no trace.
