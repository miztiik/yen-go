...
1. **No iOS canvas allocation failures** — SVG has no pixel budget limits

1. **Better incremental rendering** — GCell memoization, only changed intersections update

1. **Shadow DOM isolation** — Prevents app CSS from interfering with goban internals

1. **yengo-source bug-fix alignment** — Development effort upstream focuses on SVG renderer

1. **Better accessibility** — SVG elements in DOM tree, accessible to screen readers

### 3.3 What We Lose

1. **Initial render speed** — SVG builds ~361 `<g>` elements for 19×19 (negligible for tsumego)

1. **Memory profile** — SVG DOM nodes vs Canvas pixel buffers (negligible for tsumego)

1. **Canvas-specific CSS becomes dead** — `.ShadowLayer`, `.StoneLayer`, `.PenLayer` rules match nothing (harmless)
