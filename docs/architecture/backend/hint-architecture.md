...
1. **Do No Harm** — A misleading hint is worse than no hint. If the system cannot generate a relevant hint, emit nothing

1. **Technique-Aware** — Liberty analysis, coordinate outcome text, and diagnostic framing all adapt based on the puzzle's technique tags

1. **Transform-Invariant** — Hints use role-based labels ("Your group", not "Black") and `{!xy}` coordinate tokens (not "D16") so they survive flip/rotate/color-swap transforms

1. **Tag-Driven with Confidence-Gated Fallback** — The hint generator is primarily a function of tags and solution tree. When tags are present, all intelligence lives in the tagger. When tags are absent, the `solution_tagger` module infers technique from the correct move's board effect — but only HIGH+ confidence inferences produce hints. MEDIUM (captures) and LOW (unknown) produce coordinate-only hints

1. **Atari Relevance** — Atari hints are only emitted when the correct move actually captures the group in atari. Irrelevant atari (present on the board but unrelated to the solution) is suppressed to prevent misleading guidance
...
| **SgfBuilder** (`core/sgf_builder.py`) | Serializes hints to the pipe-delimited `YH` SGF format | Generate or interpret hints |
...
1. Classify the move's effect and assign confidence:
...
1. Only HIGH+ confidence produces technique/reasoning hints

1. MEDIUM/LOW confidence produces coordinate-only hints (YH3)
...
| **Hints** (YH) | Before move, on request | Guide toward the technique | Root `YH` with up to three pipe-delimited hints | `hint_text` (Tier 1) |
...
1. The correct move captures stones (via `Board.play()` simulation)

1. The captured stones include the group in atari
...
1. Resolve `{!xy}` tokens via `resolveHintTokens()` after board transforms

1. Display hints progressively (one tier per button click)

1. After all text hints are shown, place board marker on correct move

1. **Never pad with generated filler** — if backend provides 2 hints, show 2 text tiers + marker
...
1. YH2 was missing because `net` tag worked but `capture-race` didn't match `capture` in the dictionary

1. The student was guided to think about the WRONG concept (liberty counting instead of loose surrounding)
