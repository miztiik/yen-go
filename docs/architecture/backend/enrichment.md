...
1. Provenance (SO) tracked in pipeline state, not SGF

1. Root comments are cleaned (HTML/CJK stripped) and preserved by default; move comments standardized with Correct/Wrong prefix, using `Correct {auto-inferred}` when correctness is inferred from a lone first move

1. Source-provided YG levels are preserved if valid; classifier runs only when missing
...
1. **Source preservation** — If no collection hint but the source provides a valid `YG` slug (from `config/puzzle-levels.json`), **preserve it**

1. **Heuristic classifier** — If no collection hint and no valid source YG, run the structural heuristic

1. **Fallback** — If classification fails (practically unreachable), use `elementary` (level 3)
...
1. **Move sequences** — Ladder, ko, snapback

1. **Solution tree** — Variations, refutations

1. **Source metadata** — Tags from original
...
1. Merge with existing YL[] (if any)
...
1. **Reasoning** — Explain why it works (wrong approach warning)

1. **Coordinate** — Specific answer with technique outcome (last resort)
...
1. **No solution tree** → training

1. **Teaching root comment** → training (regex patterns from `teaching_patterns` in config)

1. **Single-move tutorial** → training (depth ≤ `max_depth` AND hc ≥ `min_comment_level` from `training_thresholds`)

1. **Curated check** → curated (quality ≥ `min_quality` AND refutations ≥ `min_refutations` AND optionally unique first move, from `curated_thresholds`)
...
1. **Normalize whitespace** — Collapse multiple spaces to single space
