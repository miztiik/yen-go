...
1. **Repeated work on resume.** Even if a file was already processed, re-running re-reads, re-hashes, and (without other guards) re-publishes it.

1. **Windows file-locking workarounds.** A per-batch JSON checkpoint flush has special-cased intervals to avoid antivirus/indexer locking.

1. **No queryable history.** "Which files failed and why?" is invisible.
...
1. Look up `rel_path` in `files`. If missing → check rename (see below); else proceed as new.

1. If `(size_bytes, mtime_ns)` match the stored row → **still re-read the file and recompute `content_hash`**.

1. If recomputed hash matches stored → confirmed unchanged → skip downstream stages, update `last_run_id` only.

1. Otherwise → treat as changed → re-ingest.
...
1. If a row exists for an old `rel_path` and that path no longer exists on disk → it is a rename. Update the row's `rel_path` to the new value, set `last_run_id`. Do not re-ingest.

1. If multiple rows match the hash → tiebreak by most recent `run_id`.

1. If both old and new paths exist on disk with the same hash → it is a copy, not a rename. Treat the new path as a new file (publish dedup will drop it as a duplicate position later).
...
1. Else if `.pm-runtime/state/<source_id>_checkpoint.json` exists → create the SQLite, seed `meta` from the JSON's `total_processed` / `total_failed` counts as historical context (no per-file rows; future walks will populate them on hash verification), then delete the JSON.
