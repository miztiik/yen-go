...
1. Define a class implementing `TechniqueDetector`

1. Add it to the `_load_detector_classes()` list in `technique_classifier.py`

1. Add tests in `tests/test_detectors_*.py`

1. The detector will be called automatically during enrichment via `get_all_detectors()`
