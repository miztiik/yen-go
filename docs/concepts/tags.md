...
1. **Comment keywords**: Whole-word English and Japanese keyword matches.

1. **Board verification**: Rule-checked detection for a small subset of techniques.
...
1. **Comment keywords** — English and Japanese keywords matched as **whole words** (no substring false positives). HIGH confidence.

1. **Board pattern analysis** — Only three techniques are board-verified:
...
1. **No fallback** — Empty tag list is valid. The previous `life-and-death` fallback was removed because not all tsumego are life-and-death problems.
...
1. `config/tags.json` defines canonical metadata.
...
1. Add to `config/tags.json` with `id`, `name`, `category`, `description`, `aliases`

1. Update adapter mappings in `tools/` modules that perform tag normalization/mapping

1. Update tagger detection in `backend/puzzle_manager/core/tagger.py` if applicable

1. Update this documentation

1. Run `pytest -m unit -k tag` to validate
