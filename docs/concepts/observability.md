...
1. **Publish log** — Each `PublishLogEntry` includes `trace_id` for cross-run querying
...
1. **puzzle_id IS in SGF files** — As the `GN` property

1. **Publish log is the permanent trace record** — Each entry includes `trace_id` for correlation

1. **Cross-run correlation uses puzzle_id** — To find same puzzle processed in different runs
