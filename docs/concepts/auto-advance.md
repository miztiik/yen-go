...
1. **Configure delay** (1–5 seconds) using the stepper that appears when the toggle is ON

1. **Solve a puzzle correctly** — a countdown ring replaces the Next button

1. After the delay expires, the next puzzle loads automatically

1. **Cancel** at any time by clicking the countdown ring or pressing `Escape`
