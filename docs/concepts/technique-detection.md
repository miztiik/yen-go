...
1. All 28 `TechniqueDetector` instances receive typed `Position`, `AnalysisResponse`, and `SolutionNode` objects

1. Each detector independently analyzes the puzzle

1. Positive results (detected=True) are collected

1. Tag slugs are deduplicated, sorted by TAG_PRIORITY, and set on the result
...
1. **No cross-detector dependencies** — prevents cascade failures

1. **Config-driven thresholds** — all parameters in `katago-enrichment.json`

1. **Graceful degradation** — DEGRADE error policy, missing data → skip (not fail)
