...
1. **No inline `style={{}}`** — Use Tailwind utility classes (exception: dynamic computed values like widths/transforms)

1. **Mode isolation** — Each page uses ONLY its own `--color-mode-{name}-*` variables

1. **Cascade over props** — Prefer CSS cascade (`data-mode`) over passing color props to every component
