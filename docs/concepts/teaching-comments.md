...
1. Within that entry, check all technique tags for `alias_comments` match

1. If no canonical match, scan alias tables across all entries

1. Fallback to `life-and-death`
...
1. **Wrong-move branches**: Each refutation branch root whose move matches a `wrong_comments` key gets the wrong-move comment appended to its existing `C[]`

1. **Append, never overwrite**: If a node already has `C[]` content, the teaching comment is appended with `\n\n` separator

1. **Confidence gate**: If `teaching_comments` is empty (suppressed upstream), Phase 3 is a no-op
...
1. **Signal detection is heuristic**: Layer 2 signals are detected from engine data fields (policy, PV, ownership). Not all puzzles have the engine data needed to trigger signals — these fall back to V1 comments (hc:2).

1. **Vital move depth limit**: Vital move detection walks the solution tree linearly. In trees with complex branching, the vital move may not be the one with the highest pedagogical value.
...
1. **TeachingStage** reads detection results + KataGo analysis data

1. **`assemble_correct_comment()`** → V2 two-layer composition: `{technique_phrase} — {signal_phrase}.`

1. **`assemble_wrong_comment()`** → 12-condition first-match classification + optional opponent-response
