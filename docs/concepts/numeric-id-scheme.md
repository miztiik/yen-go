...
1. **Filter performance**: Integer comparison (`entry.l === 120`) is faster than string comparison.

1. **Stability**: Numeric IDs are immutable. Slug renames (display-only) don't break published data.

1. **Sparse ranges**: Gaps between IDs allow insertion without renumbering existing entries.

1. **Category grouping**: Tag ID bands (10s = objectives, 30s = tesuji, 60s = techniques) enable O(1) category detection.
