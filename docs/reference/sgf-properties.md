...
1. Adapter data is used as **hints** but enricher validates and may override

1. Provenance (SO) is tracked in pipeline state, not in published SGF

1. Root comments are metadata (removed); move comments are pedagogy (preserved)
...
1. Technique hint (e.g., "Look for a sacrifice")  

1. Specific coordinate or move hint (e.g., "cg")
...
1. **Format patterns match**: Each property value matches expected pattern

1. **Level slug valid**: YG contains a recognized level slug

1. **GN format**: Must match `YENGO-{16-hex}` pattern
