...
import type { InternalPuzzle, SolutionNode as SGFSolutionNode, Position } from '@/types/puzzle-internal';
...
function positionsToSgfCoords(positions: Position[]): string[] {
  return positions
    .map(pos => positionToSgf(pos.x, pos.y))
 ...
 */
function extractSolutionSequences(node: SGFSolutionNode, currentPath: SgfCoord[] = []): SgfCoord[][] {
...
export function adaptToPagesPuzzle(internal: InternalPuzzle): TypesPuzzle {
  ...
  const region = calculateBoardRegion(internal.boardSize, internal.blackStones, internal.whiteStones);
