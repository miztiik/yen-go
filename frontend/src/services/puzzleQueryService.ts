...
export function getPuzzlesByLevel(levelId: number): PuzzleRow[] {
  return query<PuzzleRow>(
    `${BASE_SELECT} FROM puzzles p WHERE p.level_id = ?`,
    [levelId],
  );
}
 ...
 */
export function searchCollectionsByTypes(
  searchQuery: string,
  types: string[],
): CollectionRow[] {
  const escaped = searchQuery.replace(/["\-*()^~]/g, ' ').trim();
...
export function getFilterCounts(filters: QueryFilters): FilterCounts {
  if (
    !filters.levelId && !filters.tagIds?.length && !filters.collectionId &&
    filters.quality === undefined && filters.contentType === undefined &&
...
export function getPuzzlesFiltered(
...
export function getPuzzleByHash(contentHash: string): PuzzleRow | undefined {
  const rows = query<PuzzleRow>(
    `${BASE_SELECT} FROM puzzles p WHERE p.content_hash = ?`,
    [contentHash],
  );
  return rows[0];
}
