...
export function initGoban(): void {
  ...
  setGobanCallbacks({
    ...
    watchSelectedThemes: (cb: (themes: ReturnType<typeof getSelectedThemes>) => void) => {
      const observer = new MutationObserver(() => { cb(getSelectedThemes()); });
  ...
  (MoveTree as unknown as { isobranch_colors: { strong: string; weak: string } }).isobranch_colors = { strong: "#6b7280", weak: "#d1d5db" };
