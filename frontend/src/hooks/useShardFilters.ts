/**
 * useShardFilters — Preact hook that builds filter UI options from database
...
/** Return value of the {@link useShardFilters} hook. */
export interface UseShardFiltersResult {
  /** Whether filter data has been loaded. */
  readonly isLoaded: boolean;
  /** Loading error message, if any. */
  readonly error: string | null;
  /** Filter options derived from database distributions. */
  readonly filterOptions: ShardFilterOptions;
...
const EMPTY_FILTER_OPTIONS: ShardFilterOptions = {
...
function shardKeyToFilters(key: string): QueryFilters {
 ...
 */
function buildFilterOptions(distributions: FilterDistributions): ShardFilterOptions {
  ...
  return { levelOptions, tagOptionGroups, qualityOptions, contentTypeOptions };
}
...
/**
 * useShardFilters — Builds filter UI options from database distributions.
 *
 ...
 *
 * @param shardKey - The primary query key to load data for (e.g., "c6", "l120").
 ...
 * @example
 * ```tsx
 * const { isLoaded, filterOptions, toggleLevel } = useShardFilters('c6');
...
export function useShardFilters(
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        await initDb();
        const filters = shardKeyToFilters(shardKey);
