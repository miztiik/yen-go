...
const baseBtn = 'inline-flex items-center justify-center rounded-[10px] w-[4.125rem] h-[4.125rem] transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)] focus-visible:ring-offset-1 focus-visible:ring-offset-[var(--color-bg-primary)]';
...
export function TransformBar({
  ...
  return (
    ...
    >
      <button type="button" className={btnClass(settings.flipH)} onClick={onToggleFlipH} disabled={disabled} aria-pressed={settings.flipH} aria-label="Flip horizontal" title="Flip Horizontal">
        <FlipHIcon size={30} />
      </button>
      <button type="button" className={btnClass(settings.flipV)} onClick={onToggleFlipV} disabled={disabled} aria-pressed={settings.flipV} aria-label="Flip vertical" title="Flip Vertical">
        <FlipVIcon size={30} />
      </button>
      <button type="button" className={btnClass(settings.flipDiag)} onClick={onToggleFlipDiag} disabled={disabled} aria-pressed={settings.flipDiag} aria-label="Flip diagonal" title="Flip Diagonal">
        <FlipDiagIcon size={30} />
      </button>
      <button type="button" className={btnClass(settings.rotation !== 0)} onClick={onRotateCCW} disabled={disabled} aria-label="Rotate counter-clockwise" title="Rotate CCW">
        <RotateCCWIcon size={30} />
      </button>
      <button type="button" className={btnClass(settings.rotation !== 0)} onClick={onRotateCW} disabled={disabled} aria-label="Rotate clockwise" title="Rotate CW">
        <RotateCWIcon size={30} />
      </button>
      <button type="button" className={btnClass(settings.swapColors)} onClick={onToggleSwapColors} disabled={disabled} aria-pressed={settings.swapColors} aria-label="Swap colors" title="Swap Colors">
      ...
      <button type="button" className={coordsBtnClass} onClick={onToggleCoordinates} aria-pressed={coordinateLabels} aria-label={coordinateLabels ? 'Hide coordinates' : 'Show coordinates'} title={coordinateLabels ? 'Hide Coordinates' : 'Show Coordinates'}>
        <CoordsIcon size={30} />
      </button>
