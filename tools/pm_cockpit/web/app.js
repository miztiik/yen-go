// pm_cockpit — vanilla ES module. No build step. Renders raw API payloads.
// Per principle #6 the cockpit never invents domain meaning; this script
// only transforms wire JSON into HTML.
...
function pill(variant, label, opts = {}) {
  ...
  return `<span class="pill ${cls}" ${pulse}>
    <span class="glyph"></span>${escapeHtml(label)}
  </span>`;
}
...
function confirmDialog({ title, body, verb }) {
...
function paintSystemPill() {
  const btn = $("#system-pill");
  if (SYSTEM.unreachable) {
    btn.className = "pill " + PILL_VARIANTS.error;
    btn.innerHTML = `<span class="glyph"></span>System · unreachable`;
    return;
  }
  // Severity precedence: error > warn > running > ok.
  let variant = "ok", label = "System · healthy", pulse = false;
  if (SYSTEM.lock.locked && !isOurRun()) {
    variant = "warn"; label = `System · lock held (pid ${SYSTEM.lock.holderPid ?? "?"})`;
  }
  if (SYSTEM.active && !isTerminal(SYSTEM.active.status)) {
    variant = "running"; pulse = true;
    const sub = SYSTEM.active.command?.[3] || "run";
    label = `System · ${sub} active`;
  }
  btn.className = "pill " + PILL_VARIANTS[variant];
  btn.dataset.pulse = pulse ? "true" : "false";
  btn.innerHTML = `<span class="glyph"></span>${escapeHtml(label)}`;
}
...
function paintStatusStrip() {
  ...
  if (SYSTEM.unreachable) {
    ...
    const sub = SYSTEM.active.command?.[3] || "run";
    sev = "running";
    msg = `${sub} in progress · ${SYSTEM.active.handle} · `
        + `<span data-rel-time="${escapeHtml(SYSTEM.active.started_at)}">${relTime(SYSTEM.active.started_at)}</span>`;
    action = `<a href="#run" class="strip-action">Open Live Run</a>`;
...
async function refreshActiveSnapshot() {
  try {
    ...
    if (meta) {
      meta.innerHTML = `
        <div class="grid grid-cols-[5rem_1fr] gap-x-3 gap-y-0.5 text-xs">
          <span class="text-slate-500">handle</span><span class="font-mono">${escapeHtml(a.handle)}</span>
          <span class="text-slate-500">command</span><span class="font-mono break-all text-slate-300">${escapeHtml(a.command.join(" "))}</span>
          <span class="text-slate-500">started</span><span class="font-mono" data-rel-time="${escapeHtml(a.started_at)}">${relTime(a.started_at)}</span>
          <span class="text-slate-500">pid</span><span class="font-mono">${a.pid ?? "—"}</span>
          <span class="text-slate-500">lines</span><span class="font-mono tabular-nums">${a.line_count.toLocaleString()}</span>
        </div>`;
    }
    ...
    <div class="grid lg:grid-cols-[18rem,1fr] gap-6">
      <aside class="rounded-md border border-slate-800 bg-slate-900 p-4 space-y-3 self-start">
        ...
        <div class="space-y-1.5 pt-1">
          <label class="flex items-center gap-2 text-xs"><input id="run-fresh" type="checkbox" /> --fresh</label>
        ...
        <div id="run-meta" class="pt-2 border-t border-slate-800"></div>
      </aside>
      ...
      <div class="space-y-3 min-w-0">
        ...
        <div class="flex flex-wrap items-center gap-2 text-xs">
          ...
          <div class="inline-flex rounded-md ring-1 ring-slate-700 overflow-hidden" id="log-stream">
            ...
            <button data-stream="stderr" class="px-2 py-1 hover:bg-slate-800 text-rose-300">stderr</button>
          </div>
  ...
  root.innerHTML = `
    ...
    <section class="rounded-md border border-slate-800 bg-slate-900 p-4 space-y-3">
      <div class="grid md:grid-cols-4 gap-3 text-xs">
        <label>Run ID<input    id="pl-run-id"    type="text" class="block w-full mt-1 bg-slate-950 border border-slate-700 rounded px-2 py-1 font-mono" /></label>
        <label>Puzzle ID<input id="pl-puzzle-id" type="text" class="block w-full mt-1 bg-slate-950 border border-slate-700 rounded px-2 py-1 font-mono" /></label>
        <label>Source<input    id="pl-source"    type="text" class="block w-full mt-1 bg-slate-950 border border-slate-700 rounded px-2 py-1 font-mono" /></label>
        <label>Trace ID<input  id="pl-trace-id"  type="text" class="block w-full mt-1 bg-slate-950 border border-slate-700 rounded px-2 py-1 font-mono" /></label>
        <label>Date <span class="text-slate-500">(YYYY-MM-DD)</span><input id="pl-date" type="text" class="block w-full mt-1 bg-slate-950 border border-slate-700 rounded px-2 py-1 font-mono" /></label>
        <label>From<input id="pl-from" type="text" class="block w-full mt-1 bg-slate-950 border border-slate-700 rounded px-2 py-1 font-mono" /></label>
        <label>To<input   id="pl-to"   type="text" class="block w-full mt-1 bg-slate-950 border border-slate-700 rounded px-2 py-1 font-mono" /></label>
...
const HISTORY_FILTER_KEY = "pm-cockpit:historyFilter";
...
document.addEventListener("click", async (e) => {
  ...
  // System pill → open dialog
  if (e.target.closest("#system-pill")) {
    paintSystemDialog();
...
// React to the alarm-bar "Open Live Run" anchor.
window.addEventListener("hashchange", () => {
  const t = location.hash.slice(1);
  if (t in RENDERERS) showTab(t);
});

// ---------- Boot ----------

const initialTab = (location.hash.slice(1) in RENDERERS) ? location.hash.slice(1) : "overview";
showTab(initialTab);
masterTick();
