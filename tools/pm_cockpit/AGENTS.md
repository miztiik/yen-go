...
`tools/pm_cockpit/` is a localhost browser cockpit over `backend/puzzle_manager`.
...
Run with `pytest tools/pm_cockpit/tests/ -q` from the repo root (uses the
