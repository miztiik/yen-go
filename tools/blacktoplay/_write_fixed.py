"""Helper script to write the fixed verify_hash_decode.py. Run then delete this file."""
from pathlib import Path

code = '''"""Fresh hash decode verification -- fetches puzzles from BTP API and tests round-trip.
...
def _make_board_html(r):
    ...
    for row in rows:
        for cell in row:
            ...
            parts.append('<div class="cell %s"></div>' % cc)
    parts.append("</div>")
    return "".join(parts)
...
def _make_result_html(r):
    ...
    return (
        '<div class="result %s" data-puzzle-id="%s">'
        '<div class="result-header">'
        '<span class="puzzle-id">%s</span>'
        '<span class="status-badge %s">%s</span>'
        '<span class="meta">type=%s size=%sx%s rating=%s to_play=%s</span>'
        '</div>'
        '<div class="result-body">'
        '<div class="board-container">%s</div>'
        '<div class="hash-info">'
        '<p><strong>Original:</strong> <code>%s</code></p>'
        '<p><strong>Re-encoded:</strong> <code>%s</code></p>'
        '<p><strong>Stones:</strong> %sB / %sW</p>'
        '</div></div></div>'
    ) % (
...
def main():
    ...
    for p in test_puzzles:
        print("  %s (type=%s, rating=%s)" % (p["id"], p["type"], p["rating"]))
    ...
    print("HTML report saved to %s" % html_file)
    ...
    if fc > 0:
        ...
        for r in results:
            if r["status"] == "FAIL":
                print("  %s: mismatches=%s" % (r["id"], r.get("mismatch_count", "?")))
...
Path("tools/blacktoplay/verify_hash_decode.py").write_text(code.strip() + "\n", encoding="utf-8")
