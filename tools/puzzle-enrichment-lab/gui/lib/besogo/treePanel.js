besogo.makeTreePanel = function(container, editor) {
    ...
    function rebuildNavTree() {
        ...
        pathGroup.appendChild(finishPath(path, 'black', getBranchClass(root))); // Finish and add root path
    ...
    function recursiveTreeBuild(node, x, y, nextOpen) {
        ...
        if (children.length === 0) { // Reached end of branch
            ...
            for (i = 1; i < children.length; i++) {
                ...
                // End path at beginning of branch — use branch color from C[] comment
                pathGroup.appendChild(finishPath(childPath, 'black', getBranchClass(children[i])));
            }
        }
    ...
    function makeNodeIcon(node, x, y) { // Makes a node icon for the tree
        ...
        // Enrichment Lab: add correctness ring color
        var branchClass = getBranchClass(node);
        if (branchClass && element.tagName === 'g') {
            var ring = besogo.svgEl('circle', {
                cx: svgPos(x), cy: svgPos(y), r: 48,
                fill: 'none',
                'stroke-width': 6,
                stroke: branchClass === 'branch-correct' ? '#22c55e' : '#ef4444'
            });
    ...
    function finishPath(path, color, branchClass) { // Finishes path element
        var attrs = {
            d: path,
            stroke: color,
            "stroke-width": 8,
        ...
        if (branchClass) attrs['class'] = branchClass;
        var element = besogo.svgEl("path", attrs);
        return element;
    }

    // Enrichment Lab: determine branch color class from SGF C[] comment
    function getBranchClass(node) {
        var comment = '';
        try { comment = node.comment || ''; } catch(e) {}
        if (/^Correct/i.test(comment)) return 'branch-correct';
        if (/^Wrong/i.test(comment)) return 'branch-wrong';
        if (/^Refutation/i.test(comment)) return 'branch-wrong';
        return '';
    }
