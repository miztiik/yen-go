    ...
    if (options.resize === 'auto') { // Add auto-resizing unless resize option is truthy
        ...
        resizer = function() {
            var // height = window.innerHeight, // Viewport height
                ...
                isLandscape = orient === 'landscape' || (orient === 'auto' && width >= height),
            ...
            if (isLandscape) { // Landscape mode
                ...
                // Board is square; its side = min(height, width - minPanelsWidth)
                var boardSide = Math.min(height, width - minPanelsWidth);
                if (boardSide < 100) boardSide = Math.min(height, width * 0.55);
                if (panelsDiv) {
                    panelsWidth = width - boardSide;
                    panelsDiv.style.height = height + 'px';
                    panelsDiv.style.width = panelsWidth + 'px';
                }
                boardDiv.style.height = boardSide + 'px';
                boardDiv.style.width = boardSide + 'px';
            } else { // Portrait mode
