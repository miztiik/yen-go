...
class GPFrameConfig:
    """Configuration for GP frame generation — all tunables."""
...
def apply_gp_frame(
    ...
    *,
    ...
    ko: bool = False,
    offence_to_win: int = DEFAULT_OFFENCE_TO_WIN,
    config: GPFrameConfig | None = None,
    attacker_override: Color | None = None,
) -> GPFrameResult:
    ...
    Args:
        ...
        config: Optional GPFrameConfig; overrides keyword args when provided.
        attacker_override: Force a specific attacker color, bypassing the
            geometric heuristic.  When set, the frame's solid outer wall
            will be this color.  Use when the heuristic produces wrong
            polarity (e.g., White-to-move chase puzzles with few White stones).
    ...
    if config is not None:
        ...
        komi = config.komi
        ko = config.ko
        offence_to_win = config.offence_to_win
    ...
    # Determine attacker BEFORE filling so we can apply override if given,
    # otherwise let the core algorithm use its internal heuristic.
    if attacker_override is not None:
        # Explicit override — bypass geometric heuristic entirely
        forced_black = (attacker_override == Color.BLACK)
        logger.info(
            "Frame attacker override: %s (PL=%s)",
            attacker_override.value, position.player_to_move.value,
        )
    else:
        forced_black = None  # Let _tsumego_frame_stones decide

    # Run the core algorithm with the determined attacker
    filled = _tsumego_frame_stones(
        board, komi, black_to_play, ko, margin, offence_to_win,
        attacker_override_black=forced_black,
    )
    ...
    # Determine attacker color from the algorithm (post-fill board)
    ijs = [
    ...
    if forced_black is not None:
        black_to_attack = forced_black
    elif ijs:
...
def _tsumego_frame_stones(
    ...
    offence_to_win: int,
    *,
    attacker_override_black: bool | None = None,
) -> list[list[dict]]:
    ...
    Args:
        attacker_override_black: If set, forces the attacker to Black (True)
            or White (False), bypassing the geometric heuristic.
    """
    ...
    if any(flip_spec):
        ...
        filled = _tsumego_frame_stones(
            ...
            attacker_override_black=attacker_override_black,
        )
    ...
    # Determine attacker — use override if provided, else geometric heuristic
    if attacker_override_black is not None:
        black_to_attack = attacker_override_black
    else:
        black_to_attack = _guess_black_to_attack(extrema, sizes)
