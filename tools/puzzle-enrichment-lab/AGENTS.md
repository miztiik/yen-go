...
- `tests/fixtures/extended-benchmark/` — difficulty-stratified SGFs for top-5 techniques (life-and-death, ko, snapback, ladder, nakade). 13 SGFs with naming `{technique}_{level}_{source_id}.sgf`. Sourced from yengo-source. Validated by `test_fixture_integrity.py::TestExtendedBenchmarkPopulation`.
