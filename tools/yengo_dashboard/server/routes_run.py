"""Mutating + streaming HTTP routes for yengo_dashboard.
...
def _build_args(req: RunStartRequest) -> list[str]:
    ...
    Note: ``fresh`` is handled separately as a pre-clean step (not a CLI flag).
    """
    args: list[str] = ["run"]
...
def build_run_router(*, controller: RunController, runner: PipelineRunner | None = None) -> APIRouter:
    ...
    def start_run(body: RunStartRequest) -> RunSnapshot:
        # ``--fresh`` means "clean staging first, then run". The CLI doesn't
        # have a single --fresh flag; we implement it as `clean --target staging`
        # before launching the run subprocess. (`clean --target` is itself a
        # complete cleanup — there is no `--force` flag on this subcommand.)
        if body.fresh and runner is not None:
            clean_result = runner._run_capture(["clean", "--target", "staging"])
            if not clean_result["ok"]:
                raise HTTPException(
                    status_code=500,
                    detail=f"pre-clean failed: {clean_result['stderr'].strip()[:200]}",
                )
