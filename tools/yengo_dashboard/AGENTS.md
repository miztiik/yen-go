...
tools/yengo_dashboard/
  ...
  web/
    index.html          # tab shell (Library / Pipeline / Daily / Activity / Operations / Logs). Guide demoted from primary nav (Slice 3) — contextual docs reach via the `?` toggle in the top header that opens the right-side `#help-drawer`; `/guide/{path}` deep-link route + `#view-guide` section remain wired for backward compat.
    app.js              # vanilla ES module — fetch + render + EventSource. Every render* function MUST emit its title via viewHeader() so the .view-header typography stays pinned (Theme 0 invariant). Operations cards expose a Preview button that opens #preview-dialog via openPreviewModal() — GET /api/{op}/preview, render via PREVIEW_RENDERERS, then "Run for real" reuses startMaintenance() with dry_run=false (Theme 1e). Logs/Stage pane has a regex search box (#lg-pattern + #lg-stage/from/to/limit) that calls /api/logs/grep and renders a results table; each row's [Open] button dispatches the matching #stage-files-list button so the existing tail viewer kicks in (Theme 4c). Pipeline (history) view mounts a #failures-summary-card above the run list that loads /api/status/failures-summary?last=10 and renders top failure modes; each `.failures-row` click writes a `yengo-dashboard:logsGrepPrefill` sessionStorage entry then nav→/logs, where _renderLogsStagePane consumes-and-clears the prefill to auto-search (Theme 2b/2c). System dialog grows a Disk-footprint section sourced from /api/runtime-info, and the Operations Clean target dropdown is decorated with per-target byte estimates from the same payload (Theme 3b). Activity tab (`renderActivity`) renders a unified timeline from /api/activity with kind-filter chips (run/maintenance/publish) that drive the CLI's `--kinds` flag — no client-side merge (Theme 13b). Library/Overview view (`renderOverview`) decorates the inventory header with an integrity badge + per-issue table sourced from /api/inventory/check via `integrityBlock()` (Theme 14b). The same block carries Preview→Apply buttons (`data-inv-op` rebuild|reconcile|fix) wired by `_wireInventoryActionButtons()`; clicks open `openInventoryMutationModal()` which reuses `#preview-dialog` for a synchronous two-stage flow — POST /api/inventory/preview → render `InventoryMutationPreview` → POST /api/inventory/apply → morph body to `InventoryMutationResult` (Theme 14c3). Operations page (`renderMaintenance`) is async and awaits `/api/ops/catalog` first; card *bodies* live in `OPS_CARD_SPECS` keyed by op token but section placement (`maintenance`/`destructive`), Preview-button presence, `data-destructive`, and the per-card `title` tooltip are all driven by the catalog row — a backend-only re-classification (e.g., moving `clean` to destructive) reshapes the page without a cockpit release (Theme 16b). A document-level capture-phase click listener `_opsCatalogConfirmGuard` (installed once at boot via `_ensureOpsCatalogGuard()`) intercepts any click on a `[data-op]` button whose catalog row has `reversible === false && preview_supported === false`, blocks the view-local handler with `stopImmediatePropagation`, presents `confirmDialog({verb})` for typed-verb confirm, then re-fires the click with a `data-ops-confirmed="yes"` marker so the second pass falls through to the original handler — works on every view, not just Operations (Theme 16c). Library/Overview view also appends a `#taxonomy-section` after the inventory block; `_loadTaxonomySection()` fires `/api/tags` + `/api/levels` in parallel and `taxonomyTable()` renders side-by-side Tags/Levels tables with usage counts (Theme 5). Adapter rows in the Library list are rendered as `<a data-adapter-link="ID" href="/adapters/{ID}">` links; a document-level click listener routes to `showAdapterDetail(id)` which pushState's `/adapters/{id}` and calls `renderAdapterDetail()` — that fetches `/api/adapters/{id}/details` and renders summary tiles + recent runs table + recent failures table + config echo (`<pre>`) into `#view-adapter-detail`. `parsePath()` recognizes `/adapters/{id}` and the boot path / popstate handler restore the view on refresh (Theme 6a). After the main render `_loadIngestStateSection(adapterId)` fires `/api/adapters/{id}/ingest-state` and `ingestStateBlock()` paints a status badge (healthy/stale/empty/missing) + counts tile + failed-rows preview into `#ingest-state-section`; a `[data-ingest-reset]` button kicks off `openIngestStateResetModal()` — GET `/api/adapters/{id}/ingest-state/preview` → close, then `confirmDialog({verb: \`reset ${adapterId}\`})` typed-confirm → POST `/api/adapters/{id}/ingest-state/reset` → toast + tile refresh (Theme 6b). Library/Adapters list mounts `#adapter-validation-section` below the table; `_loadAdapterValidationSection()` fires `/api/adapter-config/validate` and `adapterValidationBlock()` paints a roll-up health pill + a `[data-adapter-validation]` table of per-source FAIL rows. Adapter Detail page also mounts `#adapter-config-schema-section`; `_loadAdapterConfigSchemaSection()` fires `/api/adapter-config/{id}` and `adapterConfigSchemaBlock()` renders adapter_kind + available_kinds + a `[data-adapter-config-schema]` field/type/requirement/description table sourced from the JSON-Schema fragment — read-only preview of what the future Theme 7b Edit form will render (Theme 7a). Theme 7b grafts an inline Edit form into the same host (`[data-adapter-config-form-host]`): toggle button reveals `[data-adapter-config-edit-form]` whose per-property inputs are typed by the schema; submit collects non-empty values into `set_pairs` and POSTs `/api/adapter-config/{id}/update`. A sibling `[data-adapter-config-remove]` button calls `confirmDialog({verb: <id>})` then POSTs `/remove` (force=false) and routes back to `#/adapters` on success — the backend refuses with 400 when the source is the active adapter (Theme 7b). Library/Adapters list also mounts `#adapter-bootstrap-section` below the validation roll-up; `_renderAdapterBootstrapSection()` paints a [data-adapter-bootstrap-form] (folder + adapter + id-prefix inputs) whose submit POSTs `/api/adapter-config/bootstrap` with `dry_run:true`, renders `[data-adapter-bootstrap-table]` with per-entry status (fresh/conflict), and reveals `[data-adapter-bootstrap-apply]` only when at least one fresh entry exists; clicking apply re-POSTs with `dry_run:false` after a typed `confirmDialog({verb:"apply"})` (Theme 7c). Operations view appends `#pipeline-config-section` after the destructive group; `_renderPipelineConfigSection()` GETs `/api/pipeline-config` and renders the `pipeline` doc into a `[data-pipeline-config-show]` `<pre>`, plus a small `[data-pipeline-config-set-form]` (dotted-key + value) whose submit POSTs `/api/pipeline-config` with `{set_pairs:[KEY=VALUE], force:false}` after a typed `confirmDialog({verb:"apply"})` (Theme 7d). Theme 8a adds a new top-level **Daily** nav (`renderDaily`) that fans out parallel GETs to `/api/daily/status?window_days=30` + `/api/daily/list` and renders `_renderDailyStatusBlock()` (badge: healthy / N missing / N stale + window/expected/generated/last-regen tiles + collapsed `<details>` for missing/stale lists) above `_renderDailyListBlock()` (`[data-daily-table]` of date · technique · puzzle_count · generated_at). Sidebar gains a `data-nav="daily"` button + `#view-daily` section. Theme 8b extends `_renderDailyListBlock()` with a per-row `[data-daily-preview-btn data-date=YYYY-MM-DD]` button that calls `_runDailyPreview(date)` → GET `/api/daily/preview?date=…` → renders `[data-daily-preview-card]` (technique / standard total / version / by-tag count) into `[data-daily-preview-target]` (read-only dry-run, no writes). Theme 8c adds a sibling `[data-daily-cancel-btn]` per row → `_runDailyCancel(date)` POSTs `/api/daily/cancel/preview` → confirmDialog with verb `cancel <date>` typed → POST `/api/daily/cancel/apply` → toast + re-render. Theme 8d appends a `[data-daily-backfill-btn]` to `_renderDailyStatusBlock()` (only when missing > 0) → `_runDailyBackfill(window_days)` POSTs `/api/daily/backfill/preview` → confirmDialog with verb `backfill` typed → POST `/api/daily/backfill/apply` → toast + re-render. Theme 9 grafts run-vs-run comparison into `renderHistory()` — each row gains a `.runs-diff-check` checkbox (capped at 2 selected, surfaced via `_runsDiffSelection`); a `[data-runs-diff-bar]` strip above the row list mounts a Compare button + clear via `_refreshRunsDiffBar()`; clicking Compare calls `_runRunsDiff(a, b)` → GET `/api/runs/diff?run_a=…&run_b=…&max_samples=20` → renders `#runs-diff-result` with three Δ-stat tiles (ingested/failed/skipped) plus three columns (Added/Removed/Common) showing counts + sample puzzle IDs. Theme 10 adds a deep-link puzzle detail SPA route at `/puzzle/{id}` — `parsePath()` recognizes the segment, `showPuzzleDetail(id)` pushState's the path and `renderPuzzleDetail()` GETs `/api/puzzle/{id}` then paints summary tiles (publish entries / daily appearances / audit rows / tags) + four sections: Lineage (publish-log chain with `latest`/`first` markers via `puzzleLineageTable()`), SGF (`puzzleSgfBlock()` with size + 1024-byte `<pre>` preview), Daily appearances (`puzzleDailyTable()`), and Audit trail (`puzzleAuditTable()`). The top header carries a `#puzzle-search-form` global search box that submits to `showPuzzleDetail(_normalizePuzzleId(input))` — strips an optional `YENGO-` prefix and lowercases before navigation. Backend SPA shell registers `/puzzle/{puzzle_id}` → `index.html` so refreshes work. Theme 11 grafts inline Taxonomy mutations into `taxonomyTable()`: every row gains a `[data-taxonomy-rename]` button that opens `openTaxonomyRenameModal({kind, oldSlug})` (`#preview-dialog` reused for a slug input form → POST `/api/tags/rename/preview` or `/api/levels/rename/preview` → `_renderTaxonomyPreviewBody()` paints the valid/invalid pill + `affected_puzzle_count` + errors), and the Tags table header carries a `[data-taxonomy-merge='tag']` button that opens `openTagMergeModal()` (sources comma-list + target → POST `/api/tags/merge/preview`). Sub-slice 4d wires the apply path: when preview returns `valid:true`, `_renderTaxonomyPreviewBody()` emits a real `[data-taxonomy-apply]` button that `_wireTaxonomyApplyButton()` binds with a typed-verb `confirmDialog({verb: targetSlug})` → POST `/api/tags/rename/apply` (or `/api/tags/merge/apply` / `/api/levels/rename/apply`) → swap host innerHTML for `_renderTaxonomyApplyBody()` (applied/refused pill + scanned/rewritten/config_updated/audit_timestamp tiles) and refresh `_loadTaxonomySection()` so the Library tables show post-rewrite usage counts. Theme 12 grafts a `_renderAdapterScaffoldSection()` form onto the Library/Adapters view (id + name + path inputs); submit POSTs `/api/adapter-scaffold/preview`, renders `_renderAdapterScaffoldPreview()` (ok/invalid pill + sources_entry JSON + files_created list + errors), and reveals an `[data-adapter-scaffold-apply]` button when the dry-run is valid; clicking apply runs `confirmDialog({verb:<id>})` then POSTs `/api/adapter-scaffold/apply` and toasts on success. UX-handoff "Equivalent CLI": `renderLiveRun()` mounts a `#run-cli-equiv-block` panel under the Start/Cancel row; `<pre id="run-cli-equiv">` is repainted on every form change by `paintCliEquivalent()` (pure builder `buildCliEquivalent(snap)` joins `python -m backend.puzzle_manager run` with `--source/--stage/--fresh/--dry-run/--source-override/--no-enrichment` from the current snapshot); `#run-cli-copy` writes the literal command to `navigator.clipboard`. `refreshSourceLock()` repaints when the active adapter loads asynchronously. UX-handoff "Inline contextual help": `helpCallout(id, {title, bodyHtml})` renders a soft-blue `<details>` panel; `_wireHelpCallouts()` (capture-phase `toggle` listener installed once at boot) persists the open/closed preference per id in `sessionStorage` so the layout doesn't re-expand on every navigation. Run, Adapters, and Operations views each mount one (`help-run` / `help-adapters` / `help-operations`) with operator-facing flag/term explanations + destructive-op risk text. Styled by `.help-callout*` in `styles.css` with a light-theme override. UX-handoff "Help drawer + demote Guide" (Slice 3): the Guide primary-nav button is removed; a `?` toggle (`#help-drawer-toggle`) in the top header opens a right-side slide-in `#help-drawer` (backdrop + ESC + close button). `_HELP_ROUTE_MAP` keys nav names → `docs/` paths (e.g., `library` → `how-to/tools/run-yengo-dashboard.md`, `operations` → `reference/puzzle-manager-cli.md`); `openHelpDrawer(routeKey)` defaults to the current `.nav-item.active`, fetches via the existing `/api/docs/file?path=…` endpoint, and renders with `marked`. The `/guide/{path}` route + `#view-guide` section stay wired so existing bookmarks still resolve.
...
| GET    | `/api/runs/diff`              | subprocess `runs-diff RUN_A RUN_B --max-samples N --json` (Theme 9) | 200 with `{raw: {ok, run_a, run_b, added_puzzles, removed_puzzles, common_count, changed_puzzles, stats_diff}}`; query: `run_a`, `run_b` (required), `max_samples` (default 20). 502 on CLI failure. |
| GET    | `/api/puzzle/{puzzle_id}`     | subprocess `puzzle-info PUZZLE_ID --json` (Theme 10) | 200 with `{raw: {ok, puzzle_id, found, publish_entries[], latest, first_publish, source_id, level, tags[], collections[], sgf, audit[], daily_appearances[]}}`. CLI strips an optional `YENGO-` prefix and lowercases. 502 on CLI failure. |
| POST   | `/api/tags/rename/preview`    | subprocess `tags rename OLD NEW --dry-run --json` (Theme 11) | 200 with `{raw: TaxonomyMutationPreview}`. Tolerates rc=2 (`valid:false` is a business outcome, not an error). 502 on other CLI failures. Body: `{old, new}`. |
| POST   | `/api/tags/rename/apply`      | subprocess `tags rename OLD NEW --apply --json` (Theme 11 4a) | 200 with `{raw}` carrying the apply payload (`{ok, op, sources, target, files_scanned, files_rewritten, config_updated, audit_timestamp, errors}`). Tolerates rc=2 (preview-rejected → `ok:false` + `errors[]`). Body: `{old, new}`. |
| POST   | `/api/tags/merge/preview`     | subprocess `tags merge SRC1 SRC2 [...] --into TARGET --dry-run --json` (Theme 11) | 200 with `{raw: TaxonomyMutationPreview}`. Body: `{sources[≥2], target}`. |
| POST   | `/api/tags/merge/apply`       | subprocess `tags merge SRC... --into TARGET --apply --json` (Theme 11 4a) | 200 with `{raw}` apply payload. Body: `{sources[≥2], target}`. |
| POST   | `/api/levels/rename/preview`  | subprocess `levels rename OLD NEW --dry-run --json` (Theme 11) | 200 with `{raw: TaxonomyMutationPreview}`. Body: `{old, new}`. |
| POST   | `/api/levels/rename/apply`    | subprocess `levels rename OLD NEW --apply --json` (Theme 11 4a) | 200 with `{raw}` apply payload. Body: `{old, new}`. |
| POST   | `/api/adapter-scaffold/preview` | subprocess `adapter-scaffold ... --dry-run --json` (Theme 12) | 200 with `{raw: {ok, op:"adapter-scaffold", dry_run, kind, id, name, files_created[], sources_entry, errors[]}}`. Body: `{id, kind?, name?, path?, force?}`. Tolerates rc=2 (validation failures are business outcomes). |
| POST   | `/api/adapter-scaffold/apply`   | subprocess `adapter-scaffold ... --json` (Theme 12) | Same shape as preview but writes the new package + sources.json entry behind `PipelineLock(run_id="adapter-scaffold")`. |
...
| POST   | `/api/adapter-config`               | subprocess `adapter-config add --json` (Theme 7b) | 200 with `{raw: {ok, action, message, source_id?}}`; 400 on CLI failure (incl. duplicate-id, schema, pipeline-locked). Body: `AdapterConfigAddRequest{id,name,adapter,config}` |
| POST   | `/api/adapter-config/{source_id}/clone`  | subprocess `adapter-config clone ID --new-id … --new-name … --json` (Theme 7b) | 200/400 same envelope as add. Body: `AdapterConfigCloneRequest{new_id,new_name}` |
| POST   | `/api/adapter-config/{source_id}/update` | subprocess `adapter-config update ID --set KEY=VALUE … --name … --json` (Theme 7b) | 200/400 same envelope. Body: `AdapterConfigUpdateRequest{set_pairs[], name?}` |
| POST   | `/api/adapter-config/{source_id}/remove` | subprocess `adapter-config remove ID [--force] --json` (Theme 7b) | 200/400 same envelope. 400 includes `code=active-source` when refusing the active adapter without `force=true`. Body: `AdapterConfigRemoveRequest{force}` |
| POST   | `/api/adapter-config/bootstrap`     | subprocess `adapter-config bootstrap --json` (Theme 7c) | 200 with `{raw: {ok, dry_run, applied, from_folder, entries[]}}`; 400 on CLI failure (e.g. `bad-folder`). Body: `AdapterConfigBootstrapRequest{from_folder, adapter, id_prefix, dry_run}` |
| GET    | `/api/pipeline-config`              | subprocess `pipeline-config show --json` (Theme 7d) | 200 with `{raw: {ok, pipeline}}`; 502 on CLI failure |
| POST   | `/api/pipeline-config`              | subprocess `pipeline-config set --set K=V … [--force] --json` (Theme 7d) | 200 with `{raw: {ok, applied, pipeline, message}}`; 400 on CLI failure (bad-set / pipeline-locked). Body: `PipelineConfigSetRequest{set_pairs[], force}` |
| GET    | `/api/daily/list`                   | subprocess `daily-list [--from --to] --json` (Theme 8a) | 200 with `{raw: {ok, db_exists, from?, to?, rows[]}}`; 502 on CLI failure. Query: `from` / `to` (YYYY-MM-DD) |
| GET    | `/api/daily/status`                 | subprocess `daily-status [--window-days N] [--stale-days M] --json` (Theme 8a) | 200 with `{raw: {ok, window, expected_dates, generated_dates, missing_dates[], stale_dates[], last_regenerated_at}}`; 502 on CLI failure. Query: `window_days` (1-365), `stale_days` (0-365) |
| GET    | `/api/daily/preview`                | subprocess `daily-preview --date DATE --json` (Theme 8b) | 200 with `{raw: {ok, db_exists, date, challenge?, failures[]}}`; 502 on CLI failure. Query: `date` (required, YYYY-MM-DD) |
| POST   | `/api/daily/cancel/preview`         | subprocess `daily-cancel ... --dry-run --json` (Theme 8c) | 200 with `{raw: {ok, dry_run, dates_affected[], puzzle_rows_affected}}`; 400 on rc=2 (bad args), 502 on other failure. Body: `DailyCancelRequest{date?, from?, to?, force}` |
| POST   | `/api/daily/cancel/apply`           | subprocess `daily-cancel ... --json` (Theme 8c) | 200 with `{raw: {..., schedule_rows_deleted}}`; 400/502 on failure. Same body as preview |
| POST   | `/api/daily/backfill/preview`       | subprocess `daily-backfill ... --dry-run --json` (Theme 8d) | 200 with `{raw: {ok, dry_run, window, missing_dates[], generated_count, failures[]}}`. Body: `DailyBackfillRequest{window_days, force}` |
| POST   | `/api/daily/backfill/apply`         | subprocess `daily-backfill ... --json` (Theme 8d) | 200 with `{raw: {..., generated_count}}`; 400/502 on failure. Same body as preview |
