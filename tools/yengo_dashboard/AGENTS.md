...
| POST   | `/api/tags/rename/preview`    | subprocess `tags rename OLD NEW --dry-run --json` (Theme 11) | 200 with `{raw: TaxonomyMutationPreview}`. Tolerates rc=2 (`valid:false` is a business outcome, not an error). 502 on other CLI failures. Body: `{old, new}`. Apply path deferred. |
| POST   | `/api/tags/merge/preview`     | subprocess `tags merge SRC1 SRC2 [...] --into TARGET --dry-run --json` (Theme 11) | 200 with `{raw: TaxonomyMutationPreview}`. Body: `{sources[≥2], target}`. Apply path deferred. |
| POST   | `/api/levels/rename/preview`  | subprocess `levels rename OLD NEW --dry-run --json` (Theme 11) | 200 with `{raw: TaxonomyMutationPreview}`. Body: `{old, new}`. Apply path deferred. |
