# yengo-dashboard — Plan

**Last Updated**: 2026-05-05
**Status**: Phase 3 complete (maintenance actions); Phase 4 polish landed; Phase G2 (inventory snapshot architecture) landed
**Correction Level**: 5 (Fundamental — new module + service architecture)

> **Living plan.** Once decisions are locked and the tool is operational end-to-end, the architectural rationale moves to `docs/architecture/tools/yengo-dashboard.md` and the operating guide to `docs/how-to/tools/run-yengo-dashboard.md`. This file then narrows to "what's next" only.
...
5. **All phases are shipping requirements.** §9's phase list is the order of work, not a scope filter. Phase 4 ("Polish + docs migration") is not optional or future-tense — the project is not done until docs are mirrored to `docs/architecture/tools/yengo-dashboard.md` and `docs/how-to/tools/run-yengo-dashboard.md` per project standards.
6. **Presentation-only boundary.** `tools/yengo_dashboard/` is a pure presentation layer over `backend/puzzle_manager/`. The cockpit is **allowed** to: (a) spawn `python -m backend.puzzle_manager …` subprocesses, (b) read JSON state files (`.pm-runtime/state/*.json`, `yengo-puzzle-collections/inventory.json`) as raw data, (c) tail log files, (d) render results as HTML/JSON/SSE. The cockpit is **not allowed** to: open `yengo-search.db` / `yengo-content.db` / per-source `.yengo-ingest.sqlite` (even read-only), parse SGF, compute hashes, classify puzzles, decide what a run-state field means, judge whether a lock is "stuck", or hold any other domain knowledge. **The "no SQLite handles" rule is a Phase G2 lesson** — Windows file-lock contention between cockpit reads and pipeline `os.replace()` / `Path.unlink()` was raising WinError 5/32 during `vacuum-db` and `clean`. The fix is a clean read/write split: the pipeline owns the SQLite handle exclusively and writes `inventory.json` after every mutation; the cockpit reads only that JSON snapshot. **If the UI needs data the pipeline does not already expose, the rule is "add a CLI subcommand or write a JSON snapshot from the pipeline first, then the cockpit calls/reads it"** — never reach across the boundary by re-implementing inside `tools/yengo_dashboard/` and never open a pipeline-owned SQLite DB. Architecturally this protects (i) the `tools/` → `backend/` no-import rule, (ii) the CLI as canonical interface, (iii) Windows compatibility, (iv) the operator's ability to verify any UI value by running the equivalent CLI command. Concretely: `state_reader.py` does `json.loads(...)` and returns shapes; it does not interpret them. Interpretation lives in `puzzle_manager`.
...
```
┌────────────────┐  HTTP/SSE  ┌─────────────────┐  Popen  ┌──────────────────────────┐
│  Browser UI    │ ─────────► │ yengo-dashboard      │ ──────► │ python -m puzzle_manager │
...
Single source of truth — no other semantic colors permitted in v0.1. Will be mirrored to `tools/yengo_dashboard/colors.md` when implementation starts; CI grep can fail the build on out-of-palette hex codes in state-bearing classes.
...
├── README.md                  ← short pointer to docs/how-to/tools/run-yengo-dashboard.md
├── server/
│   ├── __init__.py
│   ├── __main__.py            ← `python -m tools.yengo_dashboard` entry
...
2. **Directory + module** — `tools/yengo_dashboard/` (underscore). Invoked as `python -m tools.yengo_dashboard`. Pipeline subprocesses are `python -m backend.puzzle_manager` (matches `backend/puzzle_manager/CLAUDE.md`). No launcher script. Don't fight the system.
...
1. **WAL on `yengo-content.db`** — `core/content_db.py:192`, add `conn.execute("PRAGMA journal_mode=WAL")`. **Reason**: yengo_dashboard will read concurrently while the pipeline writes. Without WAL, readers and writers block each other and reader isolation is weak. Mirror the pattern from `source_ingest_db.py:218`.
...
Both are 1-line changes inside `backend/puzzle_manager/`. Each gets its own commit on a feature branch, lands before any yengo_dashboard Phase 2 work begins.
...
- `docs/architecture/tools/yengo-dashboard.md` — this file's §3, §4, §6, §7 (rationale + diagrams), with the See Also block per docs structure rules.
- `docs/how-to/tools/run-yengo-dashboard.md` — start the server, what each tab does, common operations.
- `docs/reference/yengo-dashboard-api.md` — endpoint table (or rely on FastAPI's `/docs` and link to it).
- `tools/yengo-dashboard/PLAN.md` — trimmed to "current phase + next steps" only; older content moved to `docs/archive/` if historically interesting.
