...
Run: pytest tools/pdf_to_sgf/tests/ -q --no-header --tb=short
"""
...
class TestPdfExtractor:
    ...
    def test_embedded_images_are_preferred(self):
        ...
        pages = extract_pages(PROBLEM_PDF)
        # tsumego-pdf embeds images as PNGs
        puzzle_pages = [p for p in pages if p.width > 100]
        assert all(p.source == "embedded" for p in puzzle_pages)
    ...
    def test_extracted_image_dimensions(self):
        ...
        page = pages[0]
        # Full-page embedded image should be substantial
...
class TestBoardDetector:
    ...
    def test_boards_sorted_top_to_bottom(self):
        ...
        # Boards should be sorted by y coordinate
    ...
    def test_board_recognition_produces_stones(self):
        ...
        # At least one board should have recognizable stones
        total_stones = 0
        for board in boards:
