...
> **Research question**: What is the current implementation state of refutation generation and tree building in `tools/puzzle-enrichment-lab/`, and what concrete hooks exist for the proposed improvements in `20260315-governance-synthesis.md`?
>
...
**Key gap (F-3 from 20260315-governance-synthesis.md)**: Ownership delta is requested but never used for refutation scoring. The `_get_ownership()` helper exists in `solve_position.py` but exclusively serves tree stopping conditions.
...
**Key gap (F-1 from 20260315-governance-synthesis.md)**: No multi-budget or adaptive visit schedule. Every candidate gets 100 visits regardless of ambiguity signal.
...
**Key gap (F-2 from 20260315-governance-synthesis.md)**: The FPU reduction is correct, but the forced-playout formula from the paper is absent. Low-policy vital moves may be under-explored.
...
**Key gap (F-4/F-5 from 20260315-governance-synthesis.md)**: Noise is fixed 0.08 regardless of board size. Single scan limits discovery of hidden wrong moves.
...
**Key gap (F-1 from 20260315-governance-synthesis.md)**: No `visit_allocation_mode: "adaptive"`. All nodes except forced moves get the same `tree_visits=500`. No `branch_visits` vs `continuation_visits` split.
...
**Key gap (F-7 from 20260315-governance-synthesis.md)**: Player nodes never branch. The entire "trick move discovery" is absent. Cho Chikun governance: should default to off (`rate=0.0`).
...
| **External references** | Covered via `20260315-governance-synthesis.md` governance panel analysis (KaTrain MIT-licensed patterns, KataGo PR #261, Kishimoto-Mueller paper). |
...
| Q2 | Should the research extend to the refutation stage wrapper (`refutation_stage.py`) and escalation retry logic? | A: Yes / B: No, `20260315-governance-synthesis.md` scope sufficient | B | | ❌ pending |
